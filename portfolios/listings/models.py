		...
		def search(self, searchdict={}):
				...
				if searchdict["listing_type"] == 'S':
						...
						if int(searchdict.get("lease_period", [])):
								qs = qs.filter(price_model_L__lease_period__gte=datetime.now() + timedelta(days=int(searchdict["lease_period"])*30),
											 				price_model_L__lease_period__lte=datetime.now() + timedelta(days=int(searchdict["lease_period"])*30 + 356))
