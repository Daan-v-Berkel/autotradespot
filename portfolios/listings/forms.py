...
class CardetailForm(StyledModelForm):
		class Meta:
				...
				widgets = {'manufacture_date': forms.DateInput}
...
class CarOptionsForm(StyledForm):
	options = forms.ModelMultipleChoiceField(widget=OptionsSelectMultiple,
...
class VariantForm(StyledForm):
		variant = forms.CharField(required=False, widget=forms.TextInput(attrs={'placeholder':_('extra variant details, e.g. PHEV or GT')}))
